...
| 命令 | 作用 |
|---|---|
| `/tree` | 会话树：哪条是主干、光标在哪、每个节点的 token 和花费、归档与废弃的标记、`⟵N3` 表示这个节点嫁接了谁 |
| `/raw` | 原样打印**下一轮真正要发给模型的全部内容**：系统提示、血缘、嫁接块、缓存断点，你还没敲的那句用「您即将输入的下一句」占位。不发送、不花钱 |
...
**嫁接不产生新节点**：它是下一个节点的一段前缀。登记之后 `/tree` 底部会写「已登记待嫁接」，等你说下一句，那个新节点才带着它出生，行尾标上 `⟵N3`。搬的只有**从公共祖先到那个节点**这一段（祖先本来就在你的上下文里，嫁接会被拒绝），而且不含子节点、不含工具输出。**想确认它到底搬了什么，敲 `/raw`**——那是下一轮真正要发出去的原文。
...
`/trunk pin <节点id>` 把主干换到另一条路径上。典型场景：主干走到 N10 才发现不对，你在
N3 上开的探索分支（N11、N12）才是对的——`/trunk pin N12`，主干就变成
`N1→N2→N3→N11→N12`，N4–N10 原地变成旁支。

三件事跟着变：主干负担按新路径重算、账本把 N4–N10 记成"探索节省"、老主干变成只读探索分支。

**新主干会被告知它可以改文件了**：探索分支的每一轮里都冻着一句"本分支只读"，改钉主干不会去改这些
已经冻结的历史（那会重写缓存），所以 kaipi 在**下一句输入前面追加一句**"本分支现在是主干，上面那条
只读通知作废"。这句只说一次，后代自然继承；前缀一个字节没动，缓存照样命中。`/raw` 里能看到它。
**没有任何节点被删除**，随时 `/trunk pin N10` 换回去。

`N1→N3` 那段**前缀缓存照样命中**：换主干只改"哪条路径算主干"这个记账事实，
一个字节的已冻结上下文都没动，所以命令会明确告诉你 `(no prefix changes)`。
分叉点（这里是 N3）本来就有一个缓存断点，探索期间一直在命中它。

**光标会跟过去**：钉完主干后，光标自动移到刚钉的节点上（会打印 `cursor moved to …`），
下一轮就接着主干写，不用再 `/go`。
...
一条命令刷屏几百行会把结论埋掉，所以默认只显示**前 4 行**。

终端里会告诉你还剩多少：`… 还有 8 行（/canvas 里点开看全部）`。
终端不能点开——已经打进 scrollback 的文字是死的，任何终端都改不回去。
画布里可以：点一下展开全部，再点收起。完整输出一直存在节点里，不会丢。

### 被限流怎么办

不用管，它会等。入门档的端点可能只允许**每分钟 3 次请求**，而一轮跑 6 条命令就是
7 次请求——撞上限流是常态，不是故障。kaipi 会按厂商给的 `retry-after` 退避重试
（最多 5 次，封顶 60 秒），并且打一行 `⏳ 被限流，8 秒后重试（第 2/5 次）`，
让"慢"和"卡死"能区分开。**等待期间 Esc 依然能停。**

厂商真的拒绝了（401、网络断、key 过期）也不会带走整个会话：那一轮和被 Esc 停掉一样处理
——记为已废弃、**已烧的 token 照样上账**、光标退回父节点，界面上只出现一句话。
...
- 输入框里也能敲斜杠命令；`/provider` 和 `/model` 要一步步问你，只能在终端里用；
- 敲 `/cli` 就把会话交还给终端。
...
预置了 **21 个提供商**：anthropic、openai、gemini、deepseek、kimi、glm、opencode、
qwen、openrouter、groq、ollama。

其中几家同时提供两种协议和两个地区的端点，各自是独立的预设——
比如 Moonshot 有 `kimi`（OpenAI 协议、国际）、`kimi-cn`（国内）、
`kimi-anthropic` 和 `kimi-anthropic-cn`（Anthropic 协议）；智谱同理。
`/provider` 里直接选，地址还能再改。

其他 OpenAI 兼容的服务，加一段 TOML 就能用。详见 [`docs/PROVIDERS.md`](docs/PROVIDERS.md)。
