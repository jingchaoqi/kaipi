...
一个很小的命令行 coding agent，特点只有一个：**省钱**。

## 它解决什么问题

跟 AI 聊代码，你的每一轮提问，模型都要把**前面所有的对话重新读一遍**，并且重新收一次钱。

所以走过的弯路特别贵：你试了个方案，发现不行，放弃了 —— 但那段对话还留在上下文里，
之后每问一句都要为它付一次钱。聊得越久越贵，而且越来越慢、越来越糊涂。

kaipi 让你可以把弯路**扔掉**，只把有用的结论**拖回来**。

## 三个词就够了

- **节点**：一轮对话（你问一次 + AI 干完活给出回答），是最小单位。
- **主干**：你真正在推进的那条路径。只有主干上的对话会被反复付费。
- **探索**：从任意节点岔出去的旁支，随便试。试砸了就归档（可恢复），
  试出结论了就"嫁接"回主干 —— 嫁接搬的是**结论**，不是全过程。

于是就有了 kaipi 最关心的那个数字：**主干负担（trunk burden）**，
也就是"你之后每问一句，都要重新付费的 token 量"。kaipi 每轮都会把它打印给你看。
...
| `/ledger` | 总账：一共花了多少，每条探索替主干挡下了多少 token |
...
- [`docs/CONCEPTS.md`](docs/CONCEPTS.md) —— 设计原理：节点、边、嫁接、六条不变量
