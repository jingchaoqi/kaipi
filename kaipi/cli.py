...
from kaipi.store import Log, list_sessions, sessions_dir
...
class Session:
    ...
    def _build(self, model: str) -> Provider:
        ...
        return AnthropicProvider(model, adaptive_thinking=price.adaptive_thinking)
...
def cmd_go(s: Session, ref: str) -> None:
    ...
    s.leaf = nid
    kind = "trunk" if nid == graph.trunk(s.log.state) else "exploration"
...
def run_input(s: Session, text: str) -> None:
    st = s.log.state
    exploration = bool(st.nodes) and s.leaf != graph.trunk(st)
    edges = s.pending
    for e in edges:
    ...
    typer.echo(
        f"{DIM}[{short(nid)} {kind}] ${cost:.4f}  cache read "
        f"{ledger.fmt(n.usage.cache_read)}/{ledger.fmt(n.usage.context)}  {s.burden_line()}{RESET}"
    )
    if exploration and guard.is_repo(s.cwd) and any(guard.WARNING in str(m) for m in n.payload):
        if typer.confirm(
...
def interactive(s: Session) -> None:
    try:
    ...
    typer.echo(
        ...
        f"  /restore <id>  /trunk [pin <id>]  /ledger  /quit{RESET}"
    )
    while True:
        try:
...
def _session(new: bool = False) -> Session:
